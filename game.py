...
There is a hidden trap door there.To go inside it simply input 'trap door' while in the storeroom.'""")
                ...
                with open("save_game.pickle", "wb") as f:
                    pickle.dump((self.current_room, self.inventory, self.smart, room3.items, room5.items, room10.items, room19.description), f)
                type_out_text("Game saved.")
                    
            elif command == "load":
                try:
                    with open("save_game.pickle", "rb") as f:
                        self.current_room, self.inventory, self.smart, room3.items, room5.items, room10.items, room19.description = pickle.load(f)
